...
pci=cli('host','pci','list');save('discovery-pci-def0b10-attempt2.json',pci)
...
networks=cli('network','list');save('discovery-networks-def0b10-attempt2.json',networks)
...
for n in networks:
 for layer in ('liveXML','persistentXML'):
  ...
  for ip in tree.findall('./ip'):
   ...
   network_cases.append((prefix,'network-live' if layer=='liveXML' else 'network-persistent',n['key']['resourceUUID']))
...
report=cli('network','cidr','check','--input',json.dumps(inp));save('discovery-cidrs-def0b10-attempt2.json',report)
...
pci_tui=tui(7,0,None,[b'"devices":',pci['devices'][0]['address'].encode()], 'discovery-pci-tui-def0b10-attempt2.ansi')
cidr_tui=tui(2,1,{'candidates':['198.18.249.0/24'],'planned':[{'id':'tui-planned','cidr':'198.18.249.0/24'}]},[b'"candidates":',b'"source": "planned"',b'"reserved": false'],'discovery-cidr-tui-def0b10-attempt2.ansi')
...
save('discovery-final-def0b10-attempt2.json',summary);print(json.dumps(summary),flush=True)
