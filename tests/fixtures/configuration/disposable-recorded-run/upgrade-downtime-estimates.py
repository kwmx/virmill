...
assert subprocess.check_output(['systemctl','--user','show','virmill-test-8a092b4.service','--property=WorkingDirectory','--value'],text=True).strip()==str(root)
subprocess.run(['rpm','-V','virmill','virmill-host-helper'],check=True)
subprocess.run(['systemctl','--user','stop','virmill-test-8a092b4.service'],check=True)
...
args=['systemd-run','--user','--unit=virmill-test-15a1f2c','--property=RuntimeMaxSec=7200','--property=Restart=no','--property=WorkingDirectory='+str(root)]
...
print(json.dumps({'upgrade':'passed','retainedXMLSHA256':before,'newUnit':'virmill-test-15a1f2c.service','existingJobsPreserved':True,'guestBootTested':False}),flush=True)
