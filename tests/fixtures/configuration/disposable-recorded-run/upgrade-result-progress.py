...
package=root/'packages/11a8174/virmill-0.0.0-0.dev.x86_64.rpm'
assert hashlib.sha256(package.read_bytes()).hexdigest()=='19accf2df0a490387fbcf4a86f5fac0fff7f1d156dd1b8b33e80f04780988b92'
def cli(*args):
...
assert subprocess.check_output(['systemctl','--user','show','<test-vm-login>-15a1f2c.service','--property=WorkingDirectory','--value'],text=True).strip()==str(root)
subprocess.run(['rpm','-V','virmill','virmill-host-helper'],check=True)
subprocess.run(['systemctl','--user','stop','<test-vm-login>-15a1f2c.service'],check=True)
subprocess.run(['sudo','-n','rpm','-Uvh','--replacepkgs',str(package)],check=True)
subprocess.run(['rpm','-V','virmill','virmill-host-helper'],check=True)
assert hashlib.sha256(pathlib.Path('/usr/bin/virmill').read_bytes()).hexdigest()=='12dbe35e3d54df4fe630e21aeb78747ebee0cbc06f412b87a0bb5480cc1607fb'
assert hashlib.sha256(pathlib.Path('/usr/bin/virmilld').read_bytes()).hexdigest()=='5decb1179c678d6a625cd413dc433df24c733ff3ee75f92dec6b133205aea9c0'
args=['systemd-run','--user','--unit=<test-vm-login>-11a8174','--property=RuntimeMaxSec=7200','--property=Restart=no','--property=WorkingDirectory='+str(root)]
...
for _ in range(100):
 ...
 time.sleep(.05)
r=cli('version');assert r.returncode==0;(root/'version-11a8174.json').write_text(r.stdout)
r=cli('operation','list');assert r.returncode==0
...
print(json.dumps({'upgrade':'passed','retainedXMLSHA256':before,'newUnit':'<test-vm-login>-11a8174.service','existingJobsPreserved':True,'guestBootTested':False}),flush=True)
