...
assert subprocess.check_output(['systemctl','--user','show','virmill-test-c7f8b76.service','--property=WorkingDirectory','--value'],text=True).strip()==str(root)
subprocess.run(['rpm','-V','virmill','virmill-host-helper'],check=True)
subprocess.run(['systemctl','--user','stop','virmill-test-c7f8b76.service'],check=True)
...
args=['systemd-run','--user','--unit=virmill-test-8a092b4','--property=RuntimeMaxSec=7200','--property=Restart=no','--property=WorkingDirectory='+str(root)]
...
print(json.dumps({'upgrade':'passed','retainedXMLSHA256':before,'newUnit':'virmill-test-8a092b4.service','existingJobsPreserved':True,'guestBootTested':False}),flush=True)
