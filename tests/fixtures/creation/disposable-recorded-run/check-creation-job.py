...
id=json.loads((root/'creation-accepted.json').read_text())['data']['operationID']
for label,args in [('creation-status',['operation','show',id]),('creation-result',['vm','creation','result',id])]:
 ...
 assert r.returncode==0 or (label=='creation-result' and json.loads(r.stdout)['error']['code']=='RECOVERY_REQUIRED')
