...
r=cli('vm','stop',id,'--plan');(root/'device-policy-stop-plan.json').write_text(r.stdout);print(r.stdout,flush=True);p=json.loads(r.stdout)['data']
...
r=cli('plan','apply',p['planID'],'--digest',p['planDigest'],'--idempotency-key','device-policy-kali-graceful-stop-c7f8b76-001','--ack','host-mutation','--detach');(root/'device-policy-stop-accepted.json').write_text(r.stdout);print(r.stdout,flush=True)
...
for _ in range(110):
 ...
 time.sleep(.5)
(root/'device-policy-stop-result.json').write_text(r.stdout);print(r.stdout,flush=True)
