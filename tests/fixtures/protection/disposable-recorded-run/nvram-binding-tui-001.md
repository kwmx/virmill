...
The fixture requires successful completion of `nvram-binding-native-001.py`,
...
```sh
python3 nvram-binding-tui-001.py --revision FULL_40_HEX_REVISION --deployment-sha256 FULL_64_HEX_MANIFEST_SHA256
```
...
Outputs use a newly created private directory `nvram-binding-tui-001/`; every
...
```sh
python3 tests/fixtures/protection/disposable-recorded-run/nvram-binding-tui-001.py --self-test
```
