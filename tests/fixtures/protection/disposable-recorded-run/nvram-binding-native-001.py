...
class Recipe:
    def __init__(self, arguments):
        ...
        self.output = ROOT / "nvram-binding-native-001"
        ...
        self.disks = None
        self.report = {
            "recipe": "nvram-binding-native-001", "revision": arguments.revision,
    ...
    def pool_volumes(self):
        names = self.virsh("vol-list", "--pool", POOL_ID, "--name").decode().splitlines()
        names = [name for name in names if name]
        require(len(names) == len(set(names)) and len(names) <= 32, "ambiguous/excessive probe-pool inventory")
    ...
    def fault(self):
        ...
        self.trigger_name = "nvram_binding_native_001_" + plan_id.replace("-", "")
        self.fault_message = "nvram-binding-native-001 receipt publication " + plan_id
        self.trigger_sql = (
    ...
    def run(self):
        ...
        hardware["name"] = "Virmill NVRAM binding native 001"
        ...
        arguments = ["plan", "apply", plan["planID"], "--digest", plan["planDigest"],
                     "--idempotency-key", "nvram-binding-native-001-" + plan["planID"]]
        ...
        b = str(pathlib.Path(a).parent / ("virmill-nvram-binding-" + self.vm + "-B.qcow2"))
