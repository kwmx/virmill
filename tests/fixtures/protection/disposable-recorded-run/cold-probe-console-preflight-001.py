...
vm='f78674f3-bf3a-43e5-81f9-4283e2472024';observer=root/'sources/uefi-probe-observer-v1/observe_console.py'
assert hashlib.sha256(observer.read_bytes()).hexdigest()=='ffa5ce3ebaaeb2360fb838d68b8895bc24aa62e84751034fae9ba02f1c2d58f4'
...
p=subprocess.run(['python3',str(observer),'--uuid',vm,'--uri','qemu:///system','--expect','SEEDED','--log',str(root/'cold-probe-console-preflight.raw'),'--timeout','2','--attempts','1'],text=True,capture_output=True,timeout=5)
observation=json.loads(p.stdout)
with (root/'cold-probe-console-preflight.json').open('x') as f:json.dump(observation,f,indent=2)
