...
Run --self-test locally for synthetic terminal/child seams only. Normal mode reads
the successful nvram-binding-native-001 artifacts and never applies a plan.
"""
...
class Recipe:
    def __init__(self, arguments):
        ...
        self.output = ROOT / 'nvram-binding-tui-001'
        self.native = ROOT / 'nvram-binding-native-001'
        ...
        self.report = {'recipe': 'nvram-binding-tui-001', 'revision': arguments.revision,
    ...
    def inspect(self):
        ...
        require(native['status'] == 'passed' and native['failures'] == [] and native['scopedTriggerRemoved'] is True
                and native['remainingLocks'] == [] and native['revision'] == self.args.revision
                and native['deploymentManifestSHA256'] == self.args.deployment_sha256, 'native restoration/reconciliation did not finish cleanly')
        ...
        require(set(self.guest_before) == set(prior_guests) | {vm_id}, 'guest inventory differs from successful native fixture')
        ...
        require(self.guest_before[vm_id][1] == read_regular(self.native / 'native-A.xml'), 'restored native A differs')
        ...
        self.report.update(planID=plan_id, planDigest=plan['planDigest'], operationID=job_id, vmID=vm_id,
                           firstPath=native['firstPath'], firstBindingSHA256=sha(binding_raw), runtime=runtime,
                           sharedCLIAndCurrentTUIEnvelopeVerified=True)
    ...
    def finish(self):
        ...
        self.report['status'] = 'passed' if not self.report['failures'] and self.report.get('sharedCLIAndCurrentTUIEnvelopeVerified') else 'inconclusive'
