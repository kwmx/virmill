...
RECIPE = 'auxiliary-inspection-native-001'
STATE_ROOT = '/var/lib/virmill-host-helper/auxiliary-fixture-001'
ROOT_ID = 'auxiliary-fixture-001'
VM_NAME = 'virmill-auxiliary-fixture-001'
URI = 'qemu:///system'
...
ROOT = '/var/lib/virmill-host-helper/auxiliary-fixture-001'
POLICY = '/etc/virmill/helper-policy.json'
...
if action=='policy-read':
    ...
    parent=directory('/etc/virmill'); name='.auxiliary-fixture-001-'+r['phase']
    fd=None
    try:
...
class Run:
    ...
    def pool_snapshot(self):
        ...
        for pool in uuid_list(self.virsh('pool-list', '--all', '--uuid')):
            ...
            volumes = {name: self.virsh('vol-dumpxml', '--pool', pool, name) for name in sorted(names)}
    ...
    def preflight(self):
        ...
        capabilities = xml_tree(self.virsh('domcapabilities', '--virttype', 'kvm', '--arch', 'x86_64', '--machine', 'pc-q35-10.2'))
    ...
    def execute(self):
        ...
        self.inspect('denied-special-file.json', expected='UNSUPPORTED_CAPABILITY')
...
class SelfTests(unittest.TestCase):
    ...
    def test_machine_refusal(self):
        ...
        denied = b'{"apiVersion":"virmill/v1","error":{"code":"PERMISSION_DENIED"}}\n'
        self.assertEqual(response_record(4, denied, b'', 'PERMISSION_DENIED')['error']['code'], 'PERMISSION_DENIED')
