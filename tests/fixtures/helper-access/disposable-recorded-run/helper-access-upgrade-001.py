...
assert subprocess.check_output(['systemctl','--user','show','virmill-test-12d7bba.service','-p','WorkingDirectory','--value'],text=True).strip()==str(root)
subprocess.run(['rpm','-V','virmill','virmill-host-helper'],check=True)
subprocess.run(['systemctl','--user','stop','virmill-test-12d7bba.service'],check=True)
...
args=['systemd-run','--user','--unit=virmill-test-b7fe053','--property=RuntimeMaxSec=7200','--property=Restart=no','--property=WorkingDirectory='+str(root)]
...
report={'upgrade':'passed','newUnit':'virmill-test-b7fe053.service','existingJobsPreserved':len(before_jobs),'resourceLocks':0,'allFourStoppedVMXMLSHA256':xml,'noGuestStartOrDefinition':True,'SELinux':subprocess.check_output(['getenforce'],text=True).strip()}
