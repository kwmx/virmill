...
try:
 ...
 wait_for(b'storage access revoke')
 for _ in range(3):os.write(master,b'\x1b[B');drain()
 os.write(master,b'\r');drain();os.write(master,grant.encode()+b'\r')
 wait_for(b'Preview is immutable')
 ...
 os.close(master)
 with (root/'helper-access-revoke-tui-b7fe053.ansi').open('xb') as f:f.write(transcript)
 with (root/'helper-access-revoke-tui-b7fe053.txt').open('x') as f:f.write(re.sub(rb'\x1b\[[0-?]*[ -/]*[@-~]',b'',transcript).decode(errors='replace'))
