...
class Artifacts(unittest.TestCase):
    ...
    def test_debian_archive_structure_and_rpm_metadata(self):
        for filename in ('virmill_1.0.0~beta.2_amd64.deb','virmill-host-helper_1.0.0~beta.2_amd64.deb'):
        ...
        for filename in ('virmill-1.0.0-0.beta.2.x86_64.rpm','virmill-host-helper-1.0.0-0.beta.2.x86_64.rpm'):
    ...
    def check_tui_search(self, root, env):
        ...
        try:
            receive('Overview')
            os.write(master,b':');receive('All tools')
            os.write(master,b'/');receive('Find task:')
            os.write(master,b'policy preview');receive('Find task: policy preview')
            os.write(master,b'\r');receive('Enter Select')
            os.write(master,b'/');receive('Enter Keep matches')
            os.write(master,b'no-such-command-xyz');receive('No matching tasks.')
