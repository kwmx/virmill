...
Status: accepted for keeping firmware and TPM state, which is implemented, with
native evidence `removal-created-native-001` and `removal-uefi-native-001`
([run record](../evidence/removal-created-run.md)); deleting them on request is
proposed.
