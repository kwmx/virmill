...
```text
Usage:
  virmill operation dispose-disk-addition ID [flags]
...
Plan a full clone: copy every writable disk of a stopped VM into a new, independent VM with its own identity

```text
Usage:
  virmill vm clone ID [flags]
...
Plan one new empty disk for a stopped VM in its own storage pool

```text
Usage:
  virmill vm disk add ID [flags]
...
## `virmill vm disk grow`

Plan growing one disk of a stopped VM; partitions inside the guest are not changed

```text
Usage:
  virmill vm disk grow ID [flags]
...
## `virmill vm disk move`

Plan copying one disk of a stopped VM to another storage pool and pointing the VM at the copy

```text
Usage:
  virmill vm disk move ID [flags]
