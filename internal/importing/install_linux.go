...
func (s *InstallationService) Plan(ctx context.Context, uid uint32, r app.Request) (domain.Plan, error) {
	...
	return s.Engine.Plan(ctx, uid, "local", "import.prepare-install", resources, map[string]string{"source-set": digest}, in, []domain.Step{step}, []string{"write-import-artifacts", "offline-source-files"}, []string{"Copies selected media and creates empty independent qcow2 disks; no VM is defined or started", "Volume identifiers and SHA-256 do not prove publisher authenticity, bootability or guest compatibility", "Keep source media offline; OFD guards exclude cooperating QEMU writers only", "No filesystem, partition, unattended credentials or guest configuration is inferred; review explicit hardware, boot order and media mapping in a separate creation plan"})
}
