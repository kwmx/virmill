...
func (s *DiskSetService) Plan(ctx context.Context, uid uint32, r app.Request) (domain.Plan, error) {
	...
	return s.Engine.Plan(ctx, uid, "local", "import.prepare-disks", resources, map[string]string{"source-set": sourceDigest}, in, []domain.Step{step}, []string{"write-import-artifacts", "offline-source-files"}, []string{"Copies every selected disk to independent qcow2; original files remain unchanged", "Only explicitly selected files are exposed to the converter; no source directory mount", "Source OFD guards refuse cooperating QEMU writers; you must keep every source offline, including writers that disable or ignore image locks", "No firmware, CPU, controller or NIC configuration can be recovered from a disk alone; VM creation requires explicit hardware assumptions", "Digests establish file integrity, not publisher authenticity or guest readiness", "No VM is defined or started; use the successful operation ID in a separate VM creation plan"})
}
