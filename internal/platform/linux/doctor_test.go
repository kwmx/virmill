...
func TestDoctorAsksForPerUserLibvirtActivation(t *testing.T) {
	...
	if unactivated.ReasonCode != "LIBVIRT_SESSION_UNACTIVATED" ||
		!reflect.DeepEqual(unactivated.Alternatives, []string{"systemctl --user restart virmilld.service"}) {
		t.Fatalf("per-user libvirt not started: %+v", unactivated)
	}
	// Where the distribution ships its own per-user socket, Virmill's skips
	// itself, so the distribution's must be enabled first.
	distro := map[string]bool{"/run/libvirt/virtqemud-sock": true, "/usr/lib/systemd/user/virtqemud.socket": true}
	withUnit := byID(t, doctor(fakeHost{paths: distro, runtime: "/run/user/1000"}.host()), "libvirt")
	if withUnit.ReasonCode != "LIBVIRT_SESSION_UNACTIVATED" || len(withUnit.Alternatives) != 2 ||
		withUnit.Alternatives[0] != "systemctl --user enable --now virtqemud.socket" {
		t.Fatalf("distribution socket unit present: %+v", withUnit)
	}
