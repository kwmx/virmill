...
func actionGroup(a ui.Action) (int, string) {
	...
	case c == "vm console show" || c == "vm set" || c == "vm boot set" || c == "vm autostart" || strings.HasPrefix(c, "guest recipe ") || strings.HasPrefix(c, "guest tools "):
...
func (m Workspace) commonAction(a ui.Action) bool {
	...
	case "vm console show", "vm set", "vm boot set", "vm create", "host inspect", "network create", "network show", "network cidr check", "storage pool show", "lab validate", "snapshot show", "snapshot restore", "backup create", "backup repository init", "backup repository check", "device usb list", "host pci list", "operation show", "operation watch", "operation cancel", "plugin install", "plugin show", "plugin enable", "plugin disable", "host capabilities", "config validate":
		return true
	}
	return false
}
