...
func (m Workspace) helpLines() []string {
	...
	return []string{
		...
		row("Moving", "Tab moves between the list, buttons and sections; a button"),
		row("", "with no key: Tab, then ←/→ and Enter"),
		...
		row("", "n New VM  v display  e CPU/RAM  o boot  c capture  g guest tools"),
