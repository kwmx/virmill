...
var actionText = map[string]struct{ label, description string }{
	...
	"import prepare":                {"Prepare OVA appliance", "Convert selected appliance disks into independent copies."},
	"import prepare-disks":          {"Prepare existing disk images", "Copy an explicit disk set and its backing files."},
	"import prepare-install":        {"Prepare ISO installation", "Copy installation media and create verified empty guest disks."},
